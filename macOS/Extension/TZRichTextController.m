//
//  TZRichTextController.m
//
//  Copyright © 2018年 TozyZuo. All rights reserved.
//

#import "TZRichTextController.h"


@interface TZTextComponent : NSObject
...
@interface TZRichTextController ()
@property (nonatomic, strong) NSString *plainText;
@property (nonatomic, strong) NSColor *textColor;
@property (nonatomic, strong) NSArray<TZTextComponent *> *textComponents;
@property (nonatomic, strong) NSArray<TZTextComponent *> *linkComponents;
...
@property (nonatomic,  weak ) TZTextComponent *mouseDownComponent;
...
@implementation TZRichTextController
...
- (instancetype)initWithView:(NSView<TZRichTextViewProtocol> *)view
{
    self = [super init];
    if (self) {
...
- (void)setView:(NSView<TZRichTextViewProtocol> *)view
{
...
- (void)parseText:(NSString *)data paragraphReplacement:(NSString *)paragraphReplacement
{
    ...
    while (startCursor < length)
    {
        ...
        if (![TZRichTextController.validTags containsObject:[[text substringFromIndex:1] componentsSeparatedByString:@" "].firstObject])
        {
            lastCursor++;
        ...
        if ([text rangeOfString:@"</"].location==0)
        {
            ...
            if (position!=NSNotFound)
            {
                ...
                {
                    TZTextComponent *component = components[i];
...
//            <a href=Struct://{unique_ptr<WebKit::NavigationState, std::__1::default_delete<WebKit::NavigationState>>="__ptr_"{__compressed_pair<WebKit::NavigationState *, std::__1::default_delete<WebKit::NavigationState>>="__first_"^{NavigationState}}}/unique_ptr<WebKit::NavigationState, std::__1::default_delete<WebKit::NavigationState>>>
            ...
            TZTextComponent *component = [TZTextComponent componentWithString:nil tag:tag attributes:attributes];
...
- (NSAttributedString *)attributedTextWithComponents:(NSArray<TZTextComponent *> *)components plainText:(NSString *)plainText
{
    ...
    for (TZTextComponent *component in components)
    {
...
- (void)mouseEntered:(NSEvent *)theEvent
{
    ...
    TZTextComponent *component = theEvent.trackingArea.userInfo[@"component"];
    NSString *colorValue = component.attributes[@"color"];
    if (!colorValue) {
        for (TZTextComponent *cpt in self.textComponents) {
...
{
    NSView<TZRichTextViewProtocol> *view = self.view;
...
//            [view hitTest:[view.superview convertPoint:event.locationInWindow fromView:nil]] &&
        ...
        {
            TZTextComponent *component = nil;
            for (TZTextComponent *cpt in self.linkComponents) {
...
@implementation TZTextComponent
...
@implementation NSTextField (TZRichTextController)
